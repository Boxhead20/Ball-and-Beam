...
while(1)
   ...
   if(opcion==1)
       %PID
       ee=floor(Setpoint-distancia);
       integral=integral+ee;
       derivativo=(ee-errorpasado)/timed;
       errorpasado=ee;
       pos=((kp*ee)+(kd*derivativo)+(ki*integral));
       if(distancia-fase<Setpoint)
        if(distancia+fase>Setpoint)
            rmax=1700;
            rmin=1500;
        end
    ...
    vec = [pos distancia  Setpoint];
    disp(vec)
        fprintf(arduino,'%f',(pos));
      
   end
   if(opcion==2)
       pos=(k1*(distancia)+k2*(angle)+bias);
